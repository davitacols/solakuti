    ...
    <article className={cn(
      ...
      <div className="flex items-center justify-between gap-3 border-b border-black/10 px-4 py-3">
        <div className="min-w-0">
          <Link href={`/livescores/competition/${fixture.competition.slug}`} className="truncate text-xs font-black uppercase tracking-[0.16em] text-black/42 transition hover:text-red-600">
            {fixture.competition.name}
          </Link>
          <p className="mt-1 truncate text-[11px] font-bold text-black/35">{fixture.round_name || kickoffLabel(fixture)}</p>
        </div>
        <span className={cn("inline-flex shrink-0 items-center gap-1 px-2.5 py-1 text-[11px] font-black uppercase tracking-[0.12em]", statusTone(fixture))}>
          {isLive ? <Radio className="size-3" /> : <Clock className="size-3" />}
          {statusLabel(fixture)}
        </span>
      </div>

      <LoadingLink href={`/livescores/match/${fixture.id}`} className="block p-4">
        <div className="grid grid-cols-[minmax(0,1fr)_82px_minmax(0,1fr)] items-center gap-3">
          ...
          <div className="text-center">
            <p className={`text-3xl font-black tracking-[-0.06em] ${isLive ? "text-red-600" : "text-[#111]"}`}>
        ...
        {showDataStrip && (
          <div className="mt-4 grid gap-2 border-t border-black/8 pt-3 text-[11px] font-black uppercase tracking-[0.12em] text-black/40 sm:grid-cols-3">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-black/8 pt-3 text-xs font-bold text-black/45">
          <span>{fixture.venue || fixture.venue_detail?.name || "Venue TBC"}</span>
